...
df_monthly = df.groupby(['field_id', 'crop_type', pd.Grouper(freq='MS')]).mean().reset_index()
...
# 4. Generate Synthetic Yield (per month snapshot)
# We add a 'seasonal' component so yield varies by month (e.g., growing season vs winter)
month_val = df_monthly['month_timestamp'].dt.month
seasonal_factor = np.sin((month_val - 1) * np.pi / 6) # Peak in summer months
...
# Final calculation for monthly yield potential
...
print(f"New Dataset Shape: {df_monthly.shape}") # Should be ~600 rows
